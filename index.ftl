...
<@layout type="index" title="${options.blog_title!}" keywords="${options.seo_keywords!}" description="${options.seo_description!}" canonical="${context!}">
    <#if settings.slider_enable!true >
      	<#-- 0表示第一页才会显示 settings.slider_json_enable!true || settings.slider_tag_enable!true && --> 
        <#if posts.number == 0>
    ...
    <div id="article-no-sudoku" class="margin-top-1d5">
        <#list posts.content as post>
            <@article post,'index','null', 'index'/>
        </#list>
    </div>
    <div id="article-sudoku" class="columns is-mobile is-multiline is-hidden padding-0375 margin-top-0375">
        <#list posts.content as post>
            <div class="column is-4-desktop is-6-mobile padding-left-right-0375 clean-bottom-padding">
                <@article post,'index','null', 'indexSudoku' />
            </div>
        </#list>
    </div>
