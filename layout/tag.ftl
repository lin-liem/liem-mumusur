...
<div class="card category-title-box">
  	<div class="category-title-bg" style="background-image: url(${tag.thumbnail!''});"></div>
  	<div class="category-title">
		<h1 class="title is-5 title-letter text-shadow margin-bottom-045"><i class="fas fa-bookmark"></i>&nbsp;${tag.name}</h1>  
  		<p class=" text-shadow">${tag.slug}</p>  
  	</div>
</div>
<div id="article-no-sudoku" class="margin-top-1d5">
        <#list posts.content as post>
            <@article post,'index','null', 'tags' />
        </#list>
    </div>
    <div id="article-sudoku" class="columns is-mobile is-multiline margin-top-075 is-hidden padding-left-right-0375">
        <#list posts.content as post>
            <div class="column is-3-desktop is-6-mobile padding-left-right-0375 clean-bottom-padding">
                <@article post,'index','null', 'tagsSudoku' />
            </div>
        </#list>
    </div>
